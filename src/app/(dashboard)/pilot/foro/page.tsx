...
export default function ForumPage() {
  ...
  const handleSearch = () => {
    ...
    router.push(`/pilot/foro${params.toString() ? '?' + params.toString() : ''}`)
  ...
  const handlePostClick = (postId: string) => {
    router.push(`/pilot/foro/${postId}`)
  }

  // Check if user is pilot
  const isPilot = user?.categoria === 'FLIGHT'
    ...
    return (
      <div className="text-center py-12">
        ...
        <h2 className="text-xl font-semibold text-[#fafafa] mb-2">
          Foro exclusivo para tripulación de vuelo
        </h2>
        <p className="text-sm text-zinc-400 mb-6">
          Este espacio está reservado para pilotos y sobrecargos.
        </p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h1 className="text-xl font-bold text-[#fafafa]">Crew Lounge</h1>
          <p className="text-xs text-zinc-500">Foro interno de la tripulación</p>
        </div>
      </div>
      ...
      {/* Create post */}
      <CreatePostForm onCreated={() => fetchPosts(searchQuery || undefined)} />
